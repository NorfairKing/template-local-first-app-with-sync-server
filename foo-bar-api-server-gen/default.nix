...
, foo-bar-api-gen, foo-bar-api-server, foo-bar-api-server-data
, foo-bar-api-server-data-gen, foo-bar-client, foo-bar-data
, foo-bar-data-gen, genvalidity-hspec, hspec, http-client
...
mkDerivation {
  ...
  libraryHaskellDepends = [
    ...
    foo-bar-api-server foo-bar-api-server-data
    foo-bar-api-server-data-gen foo-bar-client foo-bar-data
    foo-bar-data-gen genvalidity-hspec hspec http-client http-types
    monad-logger persistent persistent-sqlite QuickCheck servant
    servant-auth-client servant-auth-server servant-client text warp
  ];
  testHaskellDepends = [
    base bytestring cookie foo-bar-api foo-bar-api-gen
    foo-bar-api-server foo-bar-api-server-data
